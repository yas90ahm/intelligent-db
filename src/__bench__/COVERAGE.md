...
| 3 | **Sybil flooding (free identities)** | ✅ Measured (within #1/#2) | Poison clusters in FactWorld/PoisonedRAG are Sybil floods sharing one anchor class (K strands, `#R=1`) | "Identity is priced, not prevented": echo-collapse by passport + anchor-set disjointness; a free flood never earns independence or reputation (`α≈0`) | Demoted at L=1 in every harness — the **0% ASR** FactWorld point is the clean Sybil-flood result |
...
- **Passport (per-source key)** collapses echoes: two strands from one key are never corroboration.
